...
const indicatorConfig = {
  'stable': { color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
  'shifting': { color: 'text-blue-400', bg: 'bg-blue-400/10' },
  'under pressure': { color: 'text-amber-400', bg: 'bg-amber-400/10' },
  'emotionally conflicted': { color: 'text-purple-400', bg: 'bg-purple-400/10' },
  'at risk of contradiction': { color: 'text-red-400', bg: 'bg-red-400/10' },
};
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <header className="flex items-center justify-between border-b border-zinc-800 pb-6">
        <div>
          ...
          <p className="text-zinc-400 mt-2 text-sm">Manage your cast, their roles, and relationships.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-6">
        <AnimatePresence>
          {state.characters.map((char) => {
            ...
            <motion.div
              ...
              className={`bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-sm transition-all duration-300`}
            >
              {isEditing ? (
                <div className="p-6 space-y-6">
                  <div className="flex items-center gap-4 border-b border-zinc-800 pb-4">
                    ...
                    <button onClick={() => setActiveTab('history')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors flex items-center gap-2 ${activeTab === 'history' ? 'bg-amber-500/20 text-amber-300' : 'text-zinc-400 hover:text-zinc-200'}`}><History size={16} /> History</button>
                  </div>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-sm font-sans text-zinc-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                          placeholder="Role (e.g., Protagonist, Antagonist)"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">Core Identity (Permanent Traits)</label>
                        <textarea
                      ...
                      <div>
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">Physical Description & Background</label>
                        <textarea
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                      <div className="flex items-center justify-between bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                        ...
                        <div className="text-right">
                          <span className="text-xs text-zinc-500 uppercase tracking-wider block mb-1">Current Knowledge</span>
                          <input
                            type="text"
                  ...
                  {activeTab === 'relationships' && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                      <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">General Relationship Notes</label>
                      <textarea
                      ...
                      <div className="mt-6">
                        <div className="flex items-center justify-between mb-4">
                          <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider">Dynamic Relationship Map</label>
                          <button
                            onClick={() => {
                  ...
                  {activeTab === 'history' && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                      <div className="flex items-center justify-between mb-4">
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider">State Evolution Timeline</label>
                        <button
                          onClick={() => {
                ...
                <div className="flex flex-col h-full">
                  <div className="p-6 cursor-pointer hover:bg-zinc-800/30 transition-colors" onClick={() => setExpandedId(isExpanded ? null : char.id)}>
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="inline-block px-2.5 py-0.5 bg-zinc-800 text-zinc-400 text-xs font-medium rounded-full uppercase tracking-wider">
                            {char.role}
                          </span>
                          {char.canonStatus && (
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-6 border-t border-zinc-800/50 pt-6 bg-zinc-950/30 animate-in fade-in slide-in-from-top-2">
                      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                        ...
                        <div className="xl:col-span-1 space-y-6">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
                              {char.coreIdentity || <span className="italic text-zinc-600">No core identity defined.</span>}
                            </p>
                          </div>
                          
                          <div>
                            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Background</h3>
                            <p className="text-sm text-zinc-400 leading-relaxed whitespace-pre-wrap line-clamp-4 hover:line-clamp-none transition-all">
                              {char.description || <span className="italic text-zinc-600">No description provided.</span>}
                            </p>
                          </div>
                        </div>
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                                <Activity size={12} className="text-emerald-400" /> Live Emotional Logic
                              </h3>
                              <span className="text-xs font-medium px-2 py-1 rounded-md bg-zinc-900 border border-zinc-800 text-zinc-300">
                                Pressure: <strong className={
                                  char.currentState?.pressureLevel === 'Critical' ? 'text-red-400' :
                                  char.currentState?.pressureLevel === 'High' ? 'text-amber-400' :
                                  char.currentState?.pressureLevel === 'Medium' ? 'text-blue-400' : 'text-emerald-400'
                                }>{char.currentState?.pressureLevel || 'Low'}</strong>
                              </span>
                            </div>
                            
                            <div className="grid grid-cols-2 gap-3">
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Emotional State</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.emotionalState || '—'}</p>
                              </div>
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Visible Goal</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.visibleGoal || '—'}</p>
                              </div>
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Hidden Need</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.hiddenNeed || '—'}</p>
                              </div>
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Current Fear</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.currentFear || '—'}</p>
                              </div>
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Dominant Belief</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.dominantBelief || '—'}</p>
                              </div>
                              <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 hover:border-zinc-700 transition-colors">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-500 block mb-1">Emotional Wound</span>
                                <p className="text-sm text-zinc-200">{char.currentState?.emotionalWound || '—'}</p>
                              </div>
                            </div>
                          </div>
                          ...
                          {char.dynamicRelationships && char.dynamicRelationships.length > 0 && (
                            <div>
                              <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                                <Heart size={12} className="text-pink-400" /> Dynamic Relationship Map
                              </h3>
                                ...
                                {char.dynamicRelationships.map((rel, idx) => {
                                  ...
                                  return (
                                    <div key={idx} className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/40 flex flex-col gap-2">
                                      ...
                                      <p className="text-xs text-zinc-400 leading-relaxed">{rel.dynamics}</p>
                                    </div>
                                  );
                                })}
                              </div>
                        ...
                        <div className="xl:col-span-1 space-y-6">
                          ...
                          <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-4 flex flex-col h-full max-h-[400px]">
                            <div className="flex items-center justify-between mb-4">
                              <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                                <Sparkles size={12} className="text-amber-400" /> Intelligence Audit
                              </h3>
                              <button
                                onClick={() => handleAnalyzeState(char)}
                                disabled={analyzingId === char.id}
                                className="text-[10px] font-medium uppercase tracking-wider bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 px-2.5 py-1.5 rounded transition-colors disabled:opacity-50"
                              >
                                {analyzingId === char.id ? 'Analyzing...' : 'Run Audit'}
                              </button>
                            </div>
                            
                            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar text-sm text-zinc-300">
                              {analysisResult[char.id] ? (
                                <div className="prose prose-invert prose-sm max-w-none prose-headings:text-zinc-200 prose-headings:font-medium prose-headings:text-[10px] prose-headings:uppercase prose-headings:tracking-widest prose-headings:mt-4 prose-headings:mb-2 prose-p:leading-relaxed prose-p:text-zinc-400 prose-p:text-xs">
                                  <ReactMarkdown>{analysisResult[char.id]}</ReactMarkdown>
                                </div>
                              ) : (
                                <div className="h-full flex flex-col items-center justify-center text-center text-zinc-500 space-y-3 py-8">
                          ...
                          {char.stateHistory && char.stateHistory.length > 0 && (
                            <div>
                              <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                                <History size={12} className="text-blue-400" /> State Evolution
                              </h3>
                              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                {char.stateHistory.map((history) => (
                                  <div key={history.id} className="relative pl-4 border-l-2 border-zinc-800 pb-4 last:pb-0">
                                    <div className="absolute w-2 h-2 bg-zinc-600 rounded-full -left-[5px] top-1.5 ring-4 ring-zinc-950"></div>
                                    <span className="text-[10px] font-mono text-zinc-500 block mb-1">{history.context}</span>
                                    <p className="text-xs text-zinc-300 leading-relaxed">{history.changes}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
        ...
        {state.characters.length === 0 && (
          <div className="col-span-full text-center py-20">
            ...
            <p className="text-zinc-400 text-lg">Your cast is empty.</p>
            <p className="text-zinc-500 text-sm mt-2">Add your first character to begin building your world.</p>
          </div>
        )}
      </div>
    </div>
